...
export function SidebarShell({ children, width = "w-72" }: Props) {
  ...
  return (
    <>
      ...
      >
        <button type="button" onClick={toggle} className="hidden lg:flex items-center justify-end px-3 h-10 w-full" style={{ color: "var(--text-tertiary)" }} aria-label={open ? t("sidebar.close") : t("sidebar.open")} aria-expanded={open}>
          {open ? (
      ...
      {!open && (
        <button type="button" onClick={toggle} className="fixed bottom-4 left-4 z-30 lg:hidden rounded-full w-11 h-11 flex items-center justify-center shadow-lg text-white" style={{ background: "var(--color-brand-600)" }} aria-label={t("nav.open")}>
